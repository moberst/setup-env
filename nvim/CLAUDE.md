...
- **Entry point**: `kickstart.lua` — loads config modules, sets Python host via pyenv, then requires `plugins` module.
- **Plugin manager**: `vim.pack` (built-in, requires Neovim 0.12+) — all plugins declared in `plugins/init.lua` via `vim.pack.add()`.
- **Plugin config**: Each file in `plugins/` is a plain Lua module that calls `require("plugin").setup({...})` and sets up keymaps/autocommands. No lazy.nvim spec wrappers.
...
- All plugin URLs are centralized in `plugins/init.lua`
- Individual plugin config files are plain Lua modules (no return values needed)
- Build hooks (e.g., TSUpdate, make for fzf-native) use `PackChanged` autocommand in `plugins/init.lua`
...
- blink.cmp with sources: LSP, LuaSnip, path, buffer, lazydev
- Markdown/vimwiki/yaml filetypes get papis (citations) and render-markdown sources
...
- Plugin URLs are centralized in `plugins/init.lua` — add new plugins there
- Each plugin config file in `plugins/` is a plain Lua module that sets up the plugin
- After changes, run `configure.sh` to deploy, then restart Neovim
- On first launch after adding new plugins, `vim.pack.add()` will clone them automatically
...
- The lockfile (`nvim-pack-lock.json`) is managed by vim.pack — consider version-controlling it for reproducibility
